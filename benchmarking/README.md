...
#### Does this generalize beyond one dataset? Three independent tissues, same result

One real dataset is a weak generalization claim by itself. `estimate_real_crosstalk.py` was generalized to
run against any 10x Visium dataset with a real region/phenotype annotation, and run against two more —
deliberately **not** PDAC or BRCA-PACSI (both reserved for the manuscript's own results, kept out of
benchmarking to preserve the broader-applicability argument) — reusing datasets already available locally,
no new downloads needed:

- **BRCA-COMMOT** (breast cancer Visium, `iIMPACT` manual annotation): **Tumor vs. Healthy** — tumor regions
  are a textbook coordinated multi-pathway cancer process (EMT, hypoxia, proliferation, immune evasion all
  activate together).
- **DLPFC/Maynard** (human cortex Visium, sample 151673, spatialLIBD expert annotation): **cortical gray
  matter (Layers 1-6) vs. white matter (WM)** — layer-specific neuronal identity is a rich, multi-gene-
  program-driven differentiation; WM is comparatively homogeneous (mostly myelinated axon tracts).

**A real methodological pitfall surfaced and fixed along the way**: the null must be built from a background
pool at least as large as the phenotype group, or bootstrap resampling duplicates points and corrupts the
significance-count metric (caught when testing DLPFC with gray matter, n=3098, as the "phenotype" against a
WM pool of only 513 — the artificially inflated null showed *more* significant pairs than the real data,
an artifact of the small pool getting resampled up 6x, not a real effect). Fixed by always testing the
*smaller* of the two groups directly (WM, n=513) against the *larger* pool (gray matter, n=3098) — the
well-powered direction — and reporting the sign accordingly.

**A second finding**: with DLPFC specifically, the immune-themed panel (reused as-is from the other two
tissues) is a biological mismatch — none of its 30 pathways are meaningfully active in brain tissue. Built
a second, brain-appropriate panel (NCAM signaling, voltage-gated potassium channels, glutamate release,
postsynaptic neurotransmitter reception — curated the same way `PHENOTYPE_PROGRAM` was for immune biology)
and ran both. They agree.

| Tissue | Comparison | Panel | z (strength) | z (breadth) |
|---|---|---|---|---|
| Lymph node | GC vs. non-GC | immune | +5.99 | +3.67 |
| BRCA-COMMOT | Tumor vs. Healthy | immune | +7.79 | +2.17 |
| DLPFC | Gray matter vs. WM | immune (mismatched) | +6.75 | +6.82 |
| DLPFC | Gray matter vs. WM | neural (appropriate) | +3.87 | +4.65 |

**All four tests, three independent tissues, positive.** The phenotype-differentiated region shows
significantly more multi-pathway crosstalk than its generic counterpart every single time — immune biology,
cancer biology, and neurobiology alike. This is a materially stronger claim for the paper than the single
lymph node result: "real phenotype-differentiated tissue has elevated multi-pathway crosstalk" isn't a
property of one dataset or one biological system, it recurs across independent tissues and (for DLPFC)
survives a completely different, tissue-appropriate pathway panel too.

Publication figure: `results/figures/multi_tissue_crosstalk.{pdf,png}` — forest plot of all four z-scores.
Generated by `results/figures/plot_multi_tissue_crosstalk.py`.

#### Exhaustive sweep: did we cherry-pick the two groups per dataset?

Each test above compared one hand-picked pair (GC vs. non-GC, Tumor vs. Healthy, gray matter vs. WM) out
of however many annotation categories each dataset actually has. `sweep_real_crosstalk.py` tests **every**
pairwise combination of categories with >=50 spots, not just the one used in the headline result — the
direct answer to "did you pick the two groups that happened to work."

**BRCA-COMMOT (4 categories, all 6 possible pairs) — clean, monotonic, no exceptions:**

| Smaller (tested) | Larger (background) | z (strength) | z (breadth) |
|---|---|---|---|
| Tumor (463) | Healthy (485) | +7.16 | +2.43 |
| Tumor (463) | Surrounding tumor (823) | +4.11 | +1.18 |
| Tumor (463) | Invasive (2027) | −0.21 | −0.64 |
| Healthy (485) | Surrounding tumor (823) | −4.65 | −4.80 |
| Healthy (485) | Invasive (2027) | −7.27 | −6.13 |
| Surrounding tumor (823) | Invasive (2027) | −5.87 | −2.84 |

All 6 pairs are consistent with one coherent ordering: **Healthy < Surrounding tumor < Invasive ≈ Tumor**.
Every comparison against Healthy is significantly negative (Healthy has the least crosstalk of any
category); Tumor and Invasive — the two most malignant/differentiated states — aren't distinguishable
from each other (z≈0, as expected if both represent a similarly coordinated malignant state). This is a
graded, dose-response-like relationship between how differentiated/malignant a region is and its
crosstalk level, not just a binary GC/Tumor-shaped effect — and the original Tumor-vs-Healthy pair sits
squarely inside this pattern rather than being an outlier chosen to work.

**DLPFC (7 categories, 21 possible pairs) — honestly, a more complicated picture, reported as such:**

Cortical layers 1-6 aren't ordered along a single "generic → differentiated" axis the way cancer stages
are — each is a distinct, parallel cell-type population, not a progression. So most of the 21 pairs (the
15 layer-vs-layer comparisons) are answering a different question ("do layers X and Y have intrinsically
different baseline crosstalk") than our hypothesis, and shouldn't be read as confirming or refuting it
either way. The comparisons that actually test the hypothesis are the 6 **involving WM specifically**
(gray matter's natural comparison partner):

| Tested | Background | z (strength) | z (breadth) |
|---|---|---|---|
| WM (513) | Layer3 (989) | +1.33 | −0.06 |
| WM (513) | Layer6 (692) | −5.60 | −5.92 |
| WM (513) | Layer5 (673) | +0.57 | −1.27 |
| Layer1 (273) | WM (513) | +0.29 | +0.35 |
| Layer2 (253) | WM (513) | −1.01 | −1.13 |
| Layer4 (218) | WM (513) | −1.71 | −1.17 |

Only one individual layer (Layer6, the largest gray-matter layer, n=692) reaches significance against WM
on its own; the rest are directionally mixed and non-significant. That's a real, honest limitation, not
swept under the rug — but it's also exactly what you'd expect from a power argument, not a contradiction:
individual layers (as few as 218-989 spots) are considerably smaller than the aggregated "all 6 layers
combined" group (n=3098) that the headline DLPFC result actually used, and the aggregate result **is**
robustly significant (z=+3.87 to +6.75 across both panels). Splitting a real, aggregate-level effect into
6 individually-underpowered slices and finding most of them non-significant on their own is the normal
signature of a real effect diluted by smaller sample sizes, not evidence against it — but it does mean the
DLPFC result should be described as "gray matter collectively vs. WM," not "every layer individually beats
WM," which the exhaustive sweep is what makes that distinction visible rather than assumed.

Raw sweep output: `results/crosstalk_sweep_brca_commot.csv`, `results/crosstalk_sweep_dlpfc_neural.csv`.

#### Related literature: is "phenotype = coordinated multi-pathway crosstalk" an established idea?

Yes — this isn't a claim invented for this benchmark. Citable precedent, verified (not from memory) via
literature search:

- **Hanahan & Weinberg, "Hallmarks of Cancer: The Next Generation," *Cell* 2011** (PMID 21376230; updated in
  Hanahan, "Hallmarks of Cancer: New Dimensions," *Cancer Discovery* 2022, PMID 35022204) — the canonical
  framework: malignant phenotypes arise from coordinated dysregulation across *multiple* hallmark
  pathways/processes (proliferation, angiogenesis, immune evasion, metabolic reprogramming, etc.)
  simultaneously, not one pathway shifting in isolation.
- **Barabási, Gulbahce & Loscalzo, "Network medicine: a network-based approach to human disease," *Nature
  Reviews Genetics* 2011** (PMID 21164525) — the general systems-biology principle behind the whole premise:
  disease/phenotype states reflect perturbation of interconnected network/pathway modules, not single-gene
  or single-pathway effects.
- **Pita-Juárez et al., "The Pathway Coexpression Network: Revealing Pathway Relationships," *PLoS
  Computational Biology* 2018** (PMID 29554099) — the closest direct methodological precedent: PCxN
  systematically quantifies pathway-pathway coexpression across thousands of real expression profiles,
  explicitly correcting for shared genes between pathways to avoid trivial overlap-driven correlation —
  the same principle behind restricting to disjoint-gene-membership pairs here. Confirms that measuring
  crosstalk as pathway-activity coexpression is an established, peer-reviewed quantity, not an ad hoc
  invention for this benchmark.
- **Tissue-specific supporting reviews**: "Crosstalk of Oncogenic Signaling Pathways during
  Epithelial-Mesenchymal Transition" (PMC4263086) for the EMT claim; "Integrating Multi-Omics for
  Uncovering the Architecture of Cross-Talking Pathways in Breast Cancer" (PMC4138095) directly relevant to
  the BRCA-COMMOT result; BCR-signaling-in-germinal-center-selection review (*Trends in Immunology* 2024)
  for the coordinated BCR + Tfh/CD40-NF-kB + chemokine biology underlying the GC result.
...
**A precision correction on the above, caught by follow-up scrutiny rather than assumed**: the first
write-up of sim17 didn't compute the matched 2-way baseline at the same parameters before describing the
result, which made the gap look unchanged when it had actually shrunk somewhat. The correct, matched
comparison (same n_pairs=40, same seed, only the group structure differs):

| Metric | 2-way, 50/50 (sim15@40) | 3-way, Background-majority (sim17) |
|---|---|---|
| GRAPHIST / STAN full-panel corr. | 0.442 / 0.314 | 0.437 / 0.340 |
| gap | **+0.128** | **+0.097** |
| GRAPHIST / STAN program corr. | 0.339 / 0.262 | 0.259 / 0.203 |
| gap | **+0.077** | **+0.056** |
| GRAPHIST / STAN DE-F1 | 0.909 / 0.667 | 1.00 / 0.80 |
| gap | **+0.242** | **+0.20** |

The gap shrinks on every metric — the expected direction — it just doesn't vanish or reverse. Both
methods' *absolute* DE-F1 rose despite the DE test running on far fewer spots (259 vs. 900): verified
(not just asserted) that this is because the scattered patches are spatially compact, so the smooth
background noise field varies less *within* each small patch than across a full grid-half (within-group
SD of true activity 0.84/0.68 for the compact patches vs. 0.95/1.00 for the half-split) — lower
within-group variance inflates Cohen's d for the same shift, partly compensating for the smaller sample.

#### Does the advantage survive further — sweeping the Background fraction itself?

The natural follow-up: at what Background fraction (if any) does the advantage actually disappear? Swept
patch radius (2, 3, 4, 6, 8) at fixed crosstalk dose (n_pairs=40), giving realized Background fractions
from 91% down to 36% of the 900-spot grid:

| Background % | GRAPHIST corr. | STAN corr. | gap | GRAPHIST DE-F1 | STAN DE-F1 |
|---|---|---|---|---|---|
| 91% | 0.433 | 0.336 | 0.098 | 0.80 | 0.50 |
| 81% | 0.431 | 0.333 | 0.099 | 0.91 | 0.50 |
| 71% | 0.437 | 0.340 | 0.097 | 1.00 | 0.80 |
| 53% | 0.484 | 0.399 | 0.085 | 0.91 | 0.80 |
| 36% | 0.551 | 0.493 | 0.058 | 1.00 | 0.80 |

**The gap is essentially flat (0.097-0.099) across the entire realistic range (91%-71% Background), and
only starts eroding once Background drops below ~50%** — i.e., once phenotype-differentiated tissue
becomes the *majority*, which is the less realistic direction (real phenotype axes are typically a
minority of a tissue section, not more than half of it). Absolute correlation improves for every method as
Background shrinks (more total signal, less pure-noise majority diluting the panel), but STAN's absolute
values improve faster than GRAPHIST's, which is why the gap narrows at the far end. DE-F1 is noisier across
the sweep (0.80/0.91/1.00 non-monotonically) — consistent with it being a coarser, threshold-based metric,
as noted elsewhere in this document.

Publication figure: `results/figures/background_sweep.{pdf,png}`. Raw data:
`results/task_b_background_sweep.csv`. Generated by `results/figures/plot_background_sweep.py`.

#### Are the crosstalk-density and Background-coverage axes actually contradictory?

A fair question raised directly: sim14/15 said "more crosstalk → bigger GRAPHIST advantage"; sim17/18 said
"more phenotype-differentiated *coverage* → smaller GRAPHIST advantage." Read quickly, those sound like
opposite claims about "more phenotype differentiation." They're not — they're about two different
variables that both happen to involve phenotype differentiation:

- **Crosstalk density** (sim14/15's x-axis): holding the *size* of the phenotype group fixed, how much
  coordinated multi-pathway *structure* is in the signal. This is what a linear model structurally can't
  represent, and what GRAPHIST's advantage specifically tracks.
- **Spatial coverage** (sim17/18's x-axis): holding crosstalk structure fixed, how much of the *tissue* is
  phenotype-associated vs. Background. Unrelated to structure — it's about how much of the slide has any
  signal at all.

**Settled empirically, not just argued**: reran the crosstalk-density sweep (n_pairs = 0, 15, 40, 90, 144)
a second time, now at a fixed realistic 71% Background coverage (sim17/19) instead of the original 50/50
split, and overlaid the two curves.

| n_pairs | Gap, 0% Background (original 50/50) | Gap, 71% Background (realistic) |
|---|---|---|
| 0 | −0.103 | −0.096 |
| 15 | +0.072 | +0.062 |
| **40 (peak)** | **+0.128** | **+0.097** |
...
**Same shape — rise, peak at n_pairs=40, decline — at both Background levels**, just uniformly attenuated
(there's less total signal-carrying tissue to work with at 71% Background, so absolute values are smaller
throughout). The two curves track almost exactly through n_pairs=90; they diverge only at the extreme
(144), where the realistic-coverage curve dips slightly negative while the original stays weakly positive
— a real, minor discrepancy worth naming rather than hiding, though it doesn't change the overall
confirmation. This is direct empirical evidence the two axes are orthogonal: varying one (coverage) while
holding the other (crosstalk density) fixed does not reproduce or contradict what varying the other axis
alone does — they're independent dimensions of the same problem, not two measurements of the same thing
that happen to disagree.

Publication figure: `results/figures/2d_check.{pdf,png}`. Raw data: `results/task_b_2d_check.csv`.
Generated by `results/figures/plot_2d_check.py`.
...
pipelines directly on the *real* lymph node expression (the same 30-pathway panel, `lymph_node_phenotype`
counts, log1p-CP10K normalized), then differential-activity-tests each method's own inferred pathway
activity between the real GC and non-GC spot groups (BH<0.05, |Cohen's d|>0.3 — the same convention used
throughout). The question: does GRAPHIST's real-data DE call align with established GC immunology better
than STAN's does, on the same tissue?

| Known GC-immunology pathway | GRAPHIST \|d\| | Called DE? | STAN \|d\| | Called DE? |
|---|---|---|---|---|
| Chemokine receptors bind chemokines | 1.93 | yes (rank 1/31) | 1.43 | yes (rank 1/30) |
| BCR signaling (2nd messengers) | 1.27 | yes (rank 3/31) | 0.84 | yes (rank 3/30) |
| Downstream TCR signaling | 1.23 | yes (rank 5/31) | 0.03 | **no** (rank 27/30) |
| TCR signaling | 0.35 | yes (rank 23/31) | 0.13 | **no** (rank 17/30) |
| CD3/TCR-zeta phosphorylation | 0.96 | yes (rank 7/31) | 0.20 | **no** (rank 16/30) |
| NF-kB activation (B cells) | 0.38 | yes (rank 21/31) | 0.38 | yes (rank 9/30) |
| **Total recovered** | **6/6** | | **3/6** | |

**GRAPHIST recovers all 6 known GC-relevant pathways as differentially active; STAN misses all 3
TCR-related ones entirely** — a real, biologically meaningful gap, not a marginal one (STAN's downstream-
TCR-signaling effect size is d=0.03, essentially zero). T follicular helper cell engagement via TCR is a
central, well-established arm of the GC reaction alongside BCR signaling (see the germinal-center reviews
cited below) — GRAPHIST's DE list captures that arm, STAN's doesn't.

**Checked, not assumed, before calling this a clean win**: GRAPHIST calls DE more liberally overall (23/31
pathways vs. STAN's 11/30), so the obvious worry is indiscriminate over-calling rather than real
sensitivity. Checked directly: the *enrichment* of known-true pathways within each method's DE list is
nearly identical (GRAPHIST 6/23 = 26%, STAN 3/11 = 27%) — GRAPHIST is more sensitive across the board, not
diluting hit quality to get there.

**What this adds that the earlier steps didn't have**: this is qualitative, not ground-truth quantitative
validation (real data still has no true DE-pathway labels) — but it's a genuine comparative GRAPHIST-vs-
STAN result computed directly on real, crosstalk-elevated tissue, closing the gap between "real tissue has
elevated crosstalk" (measured) and "GRAPHIST beats STAN when crosstalk is present" (synthetic) more
directly than anything else in the suite.

Publication figure: `results/figures/gc_known_biology.{pdf,png}` — grouped bar chart of \|Cohen's d\| for
all 6 pathways, GRAPHIST vs. STAN, DE-calling threshold marked. Generated by
`results/figures/plot_gc_known_biology.py`. Data/scripts: `lymph_node_real_gc_check/` (real expression +
GC labels prepared in `run_graphist.py`/`run_stan.py`'s expected format).

**Not yet run for Task B**: PaaSc, EnrichMap (both confirmed-available, not yet implemented). Held-out
gene reconstruction is also still pending.
