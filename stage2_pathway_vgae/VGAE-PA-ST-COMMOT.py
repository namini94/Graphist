...
z_dropout = 0.3
exp_paths = '/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-COMMOT/processed/Normalized_5k_BRCA_COMMOT.csv'
...
class SVEGA(nn.Module):
    ...
    def bayesian_differential(self,
        ...
        #print(z2.shape[0])
        #print(z2.shape[1])
        #z1 = self.to_latent(torch.Tensor(X1.loc[idx1].values), adj).detach().numpy()
        #z2 = self.to_latent(torch.Tensor(X2.loc[idx2].values), adj).detach().numpy()
...
class VGAEST:
    ...
    def train_without_dec(
        ...
        for _ in tqdm(range(epochs)):
            ...
            loss = self.rec_w * loss_rec + self.gcn_w * loss_gcn + self.rec_w * loss_rec_gcn
        ...
        **kwargs
        ...
        for g in group1:
            ...
            if not group2:
                ...
                #idx_g2 = dosage.index.values[dosage.iloc[:,27] == group2]
                idx_g2 =annotation_meta.index.values[annotation_meta['annot_type'].values == group2]
                name_g2 = str(group2)
            print(self.adj_norm)
            print(idx_g1)
...
adata.obsm['VGAEST'] = z
pd.DataFrame(z).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-COMMOT/Graphencode/latent/z.csv",index=True)
...
pd.DataFrame(umap_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-COMMOT/Graphencode/latent/umap.csv",index=True)
...
count = pd.read_csv('/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-COMMOT/processed/Normalized_5k_BRCA_COMMOT.csv',header=0,index_col=0)

pathway_encoded_df = pd.DataFrame(data=z, index=count.index.values,columns=pathway_list)
#print(pathway_encoded_df)
pd.DataFrame(pathway_encoded_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-COMMOT/Graphencode/latent/pathway_encoded_df.csv",index=True)

adata.obs['REACTOME_IMMUNE_SYSTEM'] = pathway_encoded_df['REACTOME_IMMUNE_SYSTEM']
adata.obs['REACTOME_GPCR_LIGAND_BINDING'] = pathway_encoded_df['REACTOME_GPCR_LIGAND_BINDING']


fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_IMMUNE_SYSTEM', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_GPCR_LIGAND_BINDING', ax=axes[1], show=False, cmap="coolwarm",s=8)
axes[0].set_title('REACTOME_IMMUNE_SYSTEM')
axes[1].set_title('REACTOME_GPCR_LIGAND_BINDING')
plt.tight_layout()
plt.show()


adata.obs['REACTOME_METABOLISM_OF_PROTEINS'] = pathway_encoded_df['REACTOME_METABOLISM_OF_PROTEINS']
...
fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_METABOLISM_OF_PROTEINS', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES', ax=axes[1], show=False, cmap="coolwarm",s=8)
axes[0].set_title('REACTOME_METABOLISM_OF_PROTEINS')
axes[1].set_title('REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES')
plt.tight_layout()
plt.show()
...
umap_df = pd.DataFrame({'UMAP-1':embedding[:,0], 'UMAP-2':embedding[:,1],
                        'cell_type':adata.obs['celltype'], 'fine_cell_type':adata.obs['fine_celltype']})

plt.figure(figsize=[5,4])
sns.scatterplot(x='UMAP-1', y='UMAP-2', hue='cell_type', data=umap_df,
...
#plt.savefig('/Users/naminiyakan/Documents/VEGA_Code/sci-plex/plots_mycode/latent_level.pdf', format='pdf', dpi=300, bbox_inches='tight')

plt.figure(figsize=[5,4])
sns.scatterplot(x='UMAP-1', y='UMAP-2', hue='fine_cell_type', data=umap_df,
                linewidth=0, alpha=0.7, s=3)
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., fontsize=12, frameon=False, markerscale=2)
plt.xlabel('UMAP-1', fontsize=12)
plt.ylabel('UMAP-2', fontsize=12)
...
labels = pd.read_csv('/Users/naminiyakan/Documents/BulkToST/Res-BRCA-COMMOT/Graphencode/latent/Mclust_labels.csv',header=0,index_col=0)
fine_labels =  pd.read_csv('/Users/naminiyakan/Documents/BulkToST/Res-BRCA-COMMOT/Graphencode/latent/Mclust_fine_labels.csv',header=0,index_col=0)

adata.obs['labels'] = pd.Categorical(labels.iloc[:,0])
adata.obs['fine_labels'] = pd.Categorical(fine_labels.iloc[:,0])
#sub_adata = adata[~pd.isnull(adata.obs['layer_guess'])]
ARI = metrics.adjusted_rand_score(adata.obs['celltype'], adata.obs['labels'])
ARI_fine = metrics.adjusted_rand_score(adata.obs['fine_celltype'], adata.obs['fine_labels'])

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='labels', ax=axes[0], show=False)
sc.pl.spatial(adata, color='fine_labels', ax=axes[1], show=False)
axes[0].set_title('Labels: (ARI=%.4f)' % ARI)
axes[1].set_title('Fine_Labels: (ARI=%.4f)' % ARI_fine)
plt.tight_layout()
plt.show()
...
da_df = my_net.Pathway_differential_activity(X=X, fdr_target=0.05, group1='Surrounding tumor', group2='Tumor', mode='vanilla')
print(da_df.head(20))
pd.DataFrame(da_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-COMMOT/Graphencode/DEPathways/Surrounding-Tumor_vanilla.csv",index=True)
