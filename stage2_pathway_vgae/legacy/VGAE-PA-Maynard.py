...
z_dropout = 0.3
exp_paths = '/Users/naminiyakan/Documents/BulkToST/Dataset/Maynard/151673/processed/Normalized_5k_151673.csv'
...
adata.obsm['VGAEST'] = z
pd.DataFrame(z).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-Maynard/Graphencode/latent/z.csv",index=True)
...
pd.DataFrame(umap_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-Maynard/Graphencode/latent/umap.csv",index=True)
...
count = pd.read_csv('/Users/naminiyakan/Documents/BulkToST/Dataset/Maynard/151673/processed/Normalized_5k_151673.csv',header=0,index_col=0)

pathway_encoded_df = pd.DataFrame(data=z, index=count.index.values,columns=pathway_list)
#print(pathway_encoded_df)
pd.DataFrame(pathway_encoded_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-Maynard/Graphencode/latent/pathway_encoded_df.csv",index=True)

adata.obs['REACTOME_COLLAGEN_FORMATION'] = pathway_encoded_df['REACTOME_COLLAGEN_FORMATION']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_COLLAGEN_FORMATION', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_COLLAGEN_FORMATION', ax=axes[1], show=False)
...
umap_df = pd.DataFrame({'UMAP-1':embedding[:,0], 'UMAP-2':embedding[:,1],
                        'Ground-truth':adata.obs['Ground-truth']})

plt.figure(figsize=[5,4])
sns.scatterplot(x='UMAP-1', y='UMAP-2', hue='Ground-truth', data=umap_df,
...
labels = pd.read_csv('/Users/naminiyakan/Documents/BulkToST/Res-Maynard/Graphencode/latent/Mclust_labels_200.csv',header=0,index_col=0)
...
kmeans = KMeans(n_clusters=7, random_state=0, n_init="auto").fit(pathway_encoded_df)
labels_KMEANS = kmeans.labels_

adata.obs['KMEANS'] = pd.Categorical(labels_KMEANS)
sub_adata = adata[~pd.isnull(adata.obs['Ground-truth'])]

ARI = metrics.adjusted_rand_score(sub_adata.obs['Ground-truth'], sub_adata.obs['labels'])
ARI_KMEANS = metrics.adjusted_rand_score(sub_adata.obs['Ground-truth'], sub_adata.obs['KMEANS'])


fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='labels', ax=axes[0], show=False)
sc.pl.spatial(adata, color='KMEANS', ax=axes[1], show=False)
axes[0].set_title('Labels: (ARI=%.4f)' % ARI)
axes[1].set_title('KMEANS Labels: (ARI=%.4f)' % ARI_KMEANS)
plt.tight_layout()
plt.show()
...
#da_df = model.differential_activity(X=X, fdr_target=0.05, group1='Belinostat (PXD101)_10.0', group2='Belinostat (PXD101)_10000.0', mode='vanilla')
#print(da_df.head(20))
#pd.DataFrame(da_df).to_csv("/Users/naminiyakan/Documents/VEGA_Code/sci-plex/DE_Pathway_SVEGA/LH_vanilla.csv",index=True)
