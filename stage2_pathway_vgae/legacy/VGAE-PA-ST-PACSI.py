...
def create_mask( feature_list,
    ...
    add_nodes = add_nodes  
    pd.DataFrame(mask).to_csv("/Users/naminiyakan/Documents/VEGA_Code/sci-plex/latent/mask.csv",index=True)
    return mask
...
################################## END OF DECODER PART #############################
input_dim = 5000   ### This is self.n_genes
dropout = 0.1  ### self.dropout_
n_gmvs=675 ### self.n_gmvs
z_dropout = 0.3
...
class SVEGA(nn.Module):
    def __init__(self, input_dim, dropout, n_gmvs, z_dropout, gmt_paths: Union[list,str] =None, add_nodes: int = 1,min_genes: int = 0,
                max_genes: int =5000,
                positive_decoder: bool = True, exp_paths: Union[list,str] =None, regularizer: str = 'mask', gcn_hidden1 = 800, p_drop=0.2 ):
        ...
        self.GCdecoder = GraphConvolution(n_gmvs, input_dim, p_drop, act=lambda x: x)
    ...
    def differential_activity(self,
        ...
        **kwargs
        ...
        """
        dosage = pd.read_csv('/Users/naminiyakan/Documents/VEGA_Code/sci-plex/metadata_Belin.csv',header=0,index_col=0)
        level=dosage.iloc[:,27]
        # Check for grouping
        if type(group1)==str:
            group1 = [group1]
        # Loop over groups
        ...
        for g in group1:
            ...
            idx_g1 = dosage.index.values[dosage.iloc[:,27] == g]
            name_g1 = str(g)
            if not group2:
                idx_g2 = ~idx_g1
                name_g2 = 'rest'
            else: 
                idx_g2 = dosage.index.values[dosage.iloc[:,27] == group2]
                name_g2 = str(group2)
            ...
            dict_gmv = OrderedDict()
            # Check if path is a string
            gmt_paths='/Users/naminiyakan/Documents/VEGA_Code/sci-plex/reactomes.gmt'
            if type(gmt_paths) == str:
                gmt_paths = [gmt_paths]
                for f in gmt_paths:
                    d_f = _read_gmt(f, sep='\t', min_g=min_genes, max_g=max_genes)
    ...
    def bayesian_differential(self,
        ...
        print(X1)
        #print(len(X))
        z = self.to_latent(torch.Tensor(X.values), adj).detach().numpy()
        z = pd.DataFrame(z , index=X.index.values)
        print(z)
        print(len(z))
        z1 = z.loc[idx1].values
        #print(len(z1))
        z2 = z.loc[idx2].values
...
graph_dict = graph_construction(adata, 12)
print(graph_dict)


annotation_meta = pd.read_csv(
    '/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/Nami-Annotations.csv',
    index_col=0
)

annotation_meta['V1'] = annotation_meta['V1'].replace({'fat': 'Fat'})
annotation_meta['V1'] = annotation_meta['V1'].replace({'necrosis': 'Necrosis'})
annotation_meta['V1'] = annotation_meta['V1'].replace({'tumor': 'Invasive Carcinoma'})
annotation_meta['V1'] = annotation_meta['V1'].replace({'immune': 'Immune Cell'})
annotation_meta['V1'] = annotation_meta['V1'].replace({'fibrous': 'Fibrous Tissue'})
annotation_meta['V1'] = annotation_meta['V1'].replace({'Not Annotated': 'na'})
...
class VGAEST:
    def __init__(
        ...
        self.model = SVEGA(self.input_dim, 
                           dropout=0.1, n_gmvs=675, z_dropout=0.3, gmt_paths=gmt_paths, add_nodes=add_nodes, min_genes=min_genes,
                max_genes=max_genes,
                positive_decoder=positive_decoder, exp_paths=exp_paths, regularizer=regularizer, gcn_hidden1 = 800, p_drop=0.2 )
    ...
    def mask_generator(self, N=1):
        ...
        for i in range(0, self.cell_num):
            ...
            # non neighbors
            total_idx = torch.range(0, self.cell_num-1, dtype=torch.float32)
            non_neighbor = total_idx[~torch.isin(total_idx, neighbor)]
            indices = torch.randperm(len(non_neighbor), dtype=torch.float32)
            random_non_neighbor = indices[:n_selected]
            list_non_neighbor.append(random_non_neighbor)
        ...
        value = torch.concat([self.adj_label.values(), torch.zeros(len(x), dtype=torch.float32)])
        adj_mask = torch.sparse_coo_tensor(indices, value)
    ...
    def train_without_dec(
        ...
        for _ in tqdm(range(epochs)):
            ...
            loss = self.rec_w * loss_rec + self.gcn_w * loss_gcn + (self.rec_w/100) * loss_rec_gcn
            print('GCN-REC-loss: ',loss_rec_gcn.detach().cpu().numpy())
            print('REC-loss: ',loss_rec.detach().cpu().numpy())
            print('GCN-loss: ',loss_gcn.detach().cpu().numpy())
    ...
    def recon(self):
        ...
        out = StandardScaler().fit_transform(X_rec)

        return out
    
    
    @torch.no_grad()
    def Pathway_differential_activity(self,
                            X ,
                            group1: Union[str,list] = None,
                            group2: Union[str,list] = None,
                            mode: str = 'change',
        ...
        **kwargs
        ...
        #immune_meta = pd.read_csv(
        #    '/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/DE_Comparison-immune(-)-immune(+).csv',
        #     index_col=0
        #)
        ...
        for g in group1:
            # get indices and compute values
            #idx_g1 = dosage.index.values[dosage.iloc[:,27] == g]
            #idx_g1 =annotation_meta.index.values[annotation_meta['V1'].values == g]
            #idx_g1 =annotation_meta.index.values[immune_meta['immune_groups_DE'].values == g]
            idx_g1 =annotation_meta.index.values[selected['x'].values == g]
            name_g1 = str(g)
            if not group2:
                idx_g2 = ~idx_g1
                name_g2 = 'rest'
                ...
                #idx_g2 = dosage.index.values[dosage.iloc[:,27] == group2]
                #idx_g2 =annotation_meta.index.values[annotation_meta['V1'].values == group2]
                #idx_g2 =annotation_meta.index.values[immune_meta['immune_groups_DE'].values == group2]
                idx_g2 =annotation_meta.index.values[selected['x'].values == group2]
                name_g2 = str(group2)
            print(self.adj_norm)
            print(idx_g1)
            res_g = self.model.bayesian_differential(X,
                                                adj = self.adj_norm,
                                                cell_idx1 = idx_g1,
                                                cell_idx2 = idx_g2,
                                                mode=mode,
...
my_net = VGAEST(adata.X, graph_dict)  
 
my_net.train_without_dec() 


z, de_feat, X_rec = my_net.process()
adata.obsm['VGAEST'] = z
pd.DataFrame(z).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/latent/z.csv",index=True)
...
embedding = reducer.fit_transform(adata.obsm['VGAEST'])

umap_df = pd.DataFrame({'UMAP-1':embedding[:,0], 'UMAP-2':embedding[:,1], 'annotations':adata.obs['annotations'],'selection':adata.obs['selection'] })
pd.DataFrame(umap_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/latent/umap.csv",index=True)

pathway_dict = _read_gmt('/Users/naminiyakan/Documents/VEGA_Code/sci-plex/reactomes.gmt')
pathway_list = list(pathway_dict.keys())+['UNANNOTATED_'+str(k) for k in range(add_nodes)]
print(pathway_list)

count = pd.read_csv('/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/processed/Normalized_5k_BRCA_PACSI.csv',header=0,index_col=0)




plt.figure(figsize=[5,4])
sns.scatterplot(x='UMAP-1', y='UMAP-2', hue='annotations', data=umap_df,
                linewidth=0, alpha=0.7, s=3)
...
plt.figure(figsize=[5,4])
sns.scatterplot(x='UMAP-1', y='UMAP-2', hue='selection', data=umap_df,
                linewidth=0, alpha=0.7, s=3)
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., fontsize=12, frameon=False, markerscale=2)
plt.xlabel('UMAP-1', fontsize=12)
plt.ylabel('UMAP-2', fontsize=12)
...
labels = pd.read_csv('/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/latent/Mclust_labels.csv',header=0,index_col=0)

adata.obs['labels'] = pd.Categorical(labels.iloc[:,0])


ARI = metrics.adjusted_rand_score(adata.obs['annotations'], adata.obs['labels'])


fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='labels', ax=axes[0], show=False)
sc.pl.spatial(adata, color='labels', ax=axes[1], show=False)
axes[0].set_title('Labels: (ARI=%.4f)' % ARI)
axes[1].set_title('Labels: (ARI=%.4f)' % ARI)
plt.tight_layout()
plt.show()
...
pathway_encoded_df = pd.DataFrame(data=z, index=count.index.values,columns=pathway_list)
#print(pathway_encoded_df)
pd.DataFrame(pathway_encoded_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/latent/pathway_encoded_df.csv",index=True)

adata.obs['REACTOME_GPCR_DOWNSTREAM_SIGNALING'] = pathway_encoded_df['REACTOME_GPCR_DOWNSTREAM_SIGNALING']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_GPCR_DOWNSTREAM_SIGNALING', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_GPCR_DOWNSTREAM_SIGNALING', ax=axes[1], show=False)
axes[0].set_title('REACTOME_GPCR_DOWNSTREAM_SIGNALING',fontsize=8)
axes[1].set_title('REACTOME_GPCR_DOWNSTREAM_SIGNALING',fontsize=8)
plt.tight_layout()
plt.show()

adata.obs['REACTOME_ADAPTIVE_IMMUNE_SYSTEM'] = pathway_encoded_df['REACTOME_ADAPTIVE_IMMUNE_SYSTEM']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_ADAPTIVE_IMMUNE_SYSTEM', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_ADAPTIVE_IMMUNE_SYSTEM', ax=axes[1], show=False)
axes[0].set_title('REACTOME_ADAPTIVE_IMMUNE_SYSTEM',fontsize=8)
axes[1].set_title('REACTOME_ADAPTIVE_IMMUNE_SYSTEM',fontsize=8)
plt.tight_layout()
plt.show()


adata.obs['REACTOME_MYOGENESIS'] = pathway_encoded_df['REACTOME_MYOGENESIS']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_MYOGENESIS', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_MYOGENESIS', ax=axes[1], show=False)
axes[0].set_title('REACTOME_MYOGENESIS',fontsize=8)
axes[1].set_title('REACTOME_MYOGENESIS',fontsize=8)
plt.tight_layout()
plt.show()


adata.obs['REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES'] = pathway_encoded_df['REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES', ax=axes[1], show=False)
axes[0].set_title('REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES',fontsize=8)
axes[1].set_title('REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES',fontsize=8)
plt.tight_layout()
plt.show()


adata.obs['REACTOME_PHOSPHOLIPID_METABOLISM'] = pathway_encoded_df['REACTOME_PHOSPHOLIPID_METABOLISM']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_PHOSPHOLIPID_METABOLISM', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_PHOSPHOLIPID_METABOLISM', ax=axes[1], show=False)
axes[0].set_title('REACTOME_PHOSPHOLIPID_METABOLISM',fontsize=8)
axes[1].set_title('REACTOME_PHOSPHOLIPID_METABOLISM',fontsize=8)
plt.tight_layout()
plt.show()


adata.obs['REACTOME_GOLGI_ASSOCIATED_VESICLE_BIOGENESIS'] = pathway_encoded_df['REACTOME_GOLGI_ASSOCIATED_VESICLE_BIOGENESIS']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_GOLGI_ASSOCIATED_VESICLE_BIOGENESIS', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_GOLGI_ASSOCIATED_VESICLE_BIOGENESIS', ax=axes[1], show=False)
axes[0].set_title('REACTOME_GOLGI_ASSOCIATED_VESICLE_BIOGENESIS',fontsize=8)
axes[1].set_title('REACTOME_GOLGI_ASSOCIATED_VESICLE_BIOGENESIS',fontsize=8)
plt.tight_layout()
plt.show()


adata.obs['REACTOME_INTERFERON_SIGNALING'] = pathway_encoded_df['REACTOME_INTERFERON_SIGNALING']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_INTERFERON_SIGNALING', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_INTERFERON_SIGNALING', ax=axes[1], show=False)
axes[0].set_title('REACTOME_INTERFERON_SIGNALING',fontsize=8)
axes[1].set_title('REACTOME_INTERFERON_SIGNALING',fontsize=8)
plt.tight_layout()
plt.show()

adata.obs['REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION'] = pathway_encoded_df['REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata, color='REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION', ax=axes[0], show=False, cmap="coolwarm",s=8)
sc.pl.spatial(adata, color='REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION', ax=axes[1], show=False)
axes[0].set_title('REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION',fontsize=8)
axes[1].set_title('REACTOME_EXTRACELLULAR_MATRIX_ORGANIZATION',fontsize=8)
plt.tight_layout()
plt.show()
...
adata_temp.obs['REACTOME_ADAPTIVE_IMMUNE_SYSTEM'] = pathway_encoded_df['REACTOME_ADAPTIVE_IMMUNE_SYSTEM']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata_temp, color='REACTOME_ADAPTIVE_IMMUNE_SYSTEM', ax=axes[0], show=True, cmap="coolwarm",spot_size=275,frameon=False)
sc.pl.spatial(adata_temp, color='REACTOME_ADAPTIVE_IMMUNE_SYSTEM', ax=axes[1], show=True,spot_size=275,frameon=False)
axes[0].set_title('REACTOME_ADAPTIVE_IMMUNE_SYSTEM',fontsize=8)
axes[1].set_title('REACTOME_ADAPTIVE_IMMUNE_SYSTEM',fontsize=8)
plt.tight_layout()
plt.show()

adata_temp.obs['REACTOME_PHOSPHORYLATION_OF_CD3_AND_TCR_ZETA_CHAINS'] = pathway_encoded_df['REACTOME_PHOSPHORYLATION_OF_CD3_AND_TCR_ZETA_CHAINS']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata_temp, color='REACTOME_PHOSPHORYLATION_OF_CD3_AND_TCR_ZETA_CHAINS', ax=axes[0], show=True, cmap="coolwarm",spot_size=275,frameon=False)
sc.pl.spatial(adata_temp, color='REACTOME_PHOSPHORYLATION_OF_CD3_AND_TCR_ZETA_CHAINS', ax=axes[1], show=True,spot_size=275,frameon=False)
axes[0].set_title('REACTOME_PHOSPHORYLATION_OF_CD3_AND_TCR_ZETA_CHAINS',fontsize=8)
axes[1].set_title('REACTOME_PHOSPHORYLATION_OF_CD3_AND_TCR_ZETA_CHAINS',fontsize=8)
plt.tight_layout()
plt.show()

adata_temp.obs['REACTOME_SIGNALING_BY_ROBO_RECEPTOR'] = pathway_encoded_df['REACTOME_SIGNALING_BY_ROBO_RECEPTOR']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata_temp, color='REACTOME_SIGNALING_BY_ROBO_RECEPTOR', ax=axes[0], show=True, cmap="coolwarm",spot_size=275,frameon=False)
sc.pl.spatial(adata_temp, color='REACTOME_SIGNALING_BY_ROBO_RECEPTOR', ax=axes[1], show=True,spot_size=275,frameon=False)
axes[0].set_title('REACTOME_SIGNALING_BY_ROBO_RECEPTOR',fontsize=8)
axes[1].set_title('REACTOME_SIGNALING_BY_ROBO_RECEPTOR',fontsize=8)
plt.tight_layout()
plt.show()

adata_temp.obs['REACTOME_SIGNALING_BY_NOTCH'] = pathway_encoded_df['REACTOME_SIGNALING_BY_NOTCH']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata_temp, color='REACTOME_SIGNALING_BY_NOTCH', ax=axes[0], show=True, cmap="coolwarm",spot_size=275,frameon=False)
sc.pl.spatial(adata_temp, color='REACTOME_SIGNALING_BY_NOTCH', ax=axes[1], show=True,spot_size=275,frameon=False)
axes[0].set_title('REACTOME_SIGNALING_BY_NOTCH',fontsize=8)
axes[1].set_title('REACTOME_SIGNALING_BY_NOTCH',fontsize=8)
plt.tight_layout()
plt.show()

adata_temp.obs['REACTOME_METABOLISM_OF_AMINO_ACIDS_AND_DERIVATIVES'] = pathway_encoded_df['REACTOME_METABOLISM_OF_AMINO_ACIDS_AND_DERIVATIVES']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata_temp, color='REACTOME_METABOLISM_OF_AMINO_ACIDS_AND_DERIVATIVES', ax=axes[0], show=True, cmap="coolwarm",spot_size=275,frameon=False)
sc.pl.spatial(adata_temp, color='REACTOME_METABOLISM_OF_AMINO_ACIDS_AND_DERIVATIVES', ax=axes[1], show=True,spot_size=275,frameon=False)
axes[0].set_title('REACTOME_METABOLISM_OF_AMINO_ACIDS_AND_DERIVATIVES',fontsize=8)
axes[1].set_title('REACTOME_METABOLISM_OF_AMINO_ACIDS_AND_DERIVATIVES',fontsize=8)
plt.tight_layout()
plt.show()


adata_temp.obs['REACTOME_VOLTAGE_GATED_POTASSIUM_CHANNELS'] = pathway_encoded_df['REACTOME_VOLTAGE_GATED_POTASSIUM_CHANNELS']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata_temp, color='REACTOME_VOLTAGE_GATED_POTASSIUM_CHANNELS', ax=axes[0], show=True, cmap="coolwarm",spot_size=275,frameon=False)
sc.pl.spatial(adata_temp, color='REACTOME_VOLTAGE_GATED_POTASSIUM_CHANNELS', ax=axes[1], show=True,spot_size=275,frameon=False)
axes[0].set_title('REACTOME_VOLTAGE_GATED_POTASSIUM_CHANNELS',fontsize=8)
axes[1].set_title('REACTOME_VOLTAGE_GATED_POTASSIUM_CHANNELS',fontsize=8)
plt.tight_layout()
plt.show()

adata_temp.obs['REACTOME_INTEGRIN_ALPHAIIB_BETA3_SIGNALING'] = pathway_encoded_df['REACTOME_INTEGRIN_ALPHAIIB_BETA3_SIGNALING']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata_temp, color='REACTOME_INTEGRIN_ALPHAIIB_BETA3_SIGNALING', ax=axes[0], show=True, cmap="coolwarm",spot_size=275,frameon=False,title=None)
sc.pl.spatial(adata_temp, color='REACTOME_INTEGRIN_ALPHAIIB_BETA3_SIGNALING', ax=axes[1], show=True,spot_size=275,frameon=False,title=None)
axes[0].set_title('A',fontsize=8)
axes[1].set_title('A',fontsize=8)
plt.tight_layout()
plt.show()


adata_temp.obs['REACTOME_INTEGRIN_CELL_SURFACE_INTERACTIONS'] = pathway_encoded_df['REACTOME_INTEGRIN_CELL_SURFACE_INTERACTIONS']

fig, axes = plt.subplots(1,2,figsize=(4*2, 4))
sc.pl.spatial(adata_temp, color='REACTOME_INTEGRIN_CELL_SURFACE_INTERACTIONS', ax=axes[0], show=True, cmap="coolwarm",spot_size=275,frameon=False,title=None)
sc.pl.spatial(adata_temp, color='REACTOME_INTEGRIN_CELL_SURFACE_INTERACTIONS', ax=axes[1], show=True,spot_size=275,frameon=False,title=None)
axes[0].set_title('B',fontsize=8)
...
custom_palette = {
    'Invasive Carcinoma': 'seagreen',
    'Fibrous Tissue': 'darkorchid',
    'Immune Cell': 'gold',
    'Fat': 'lightskyblue',
    'Necrosis': 'coral',
    'na': 'gray'
...
pathways = [
    'REACTOME_GPCR_DOWNSTREAM_SIGNALING', 'REACTOME_ADAPTIVE_IMMUNE_SYSTEM','REACTOME_MYOGENESIS',
    'REACTOME_TRANSMEMBRANE_TRANSPORT_OF_SMALL_MOLECULES','REACTOME_PHOSPHOLIPID_METABOLISM','REACTOME_GOLGI_ASSOCIATED_VESICLE_BIOGENESIS'
]

# Create a long-form DataFrame for plotting
df = adata.obs[pathways + ['annotations']].copy()
df_melted = df.melt(id_vars='annotations', var_name='Pathway', value_name='Activity')
...
plt.title('Pathway Activity Across Annotations')
plt.tight_layout()
plt.show()

#============================================== Pathway DE Acitivity ====================================================
#X = pd.read_csv('/Users/naminiyakan/Documents/VEGA_Code/sci-plex/count_Belin.csv',header=0,index_col=0)

count = adata.to_df()
X = pd.DataFrame(adata.X, columns=count.columns.values, index= count.index.values)
print(X)

###da_df = my_net.Pathway_differential_activity(X=X, fdr_target=0.05, group1='Invasive Carcinoma', group2='Fibrous Tissue', mode='vanilla')
###print(da_df.head(20))
###pd.DataFrame(da_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/DEPathways/Tumor_Fibrous.csv",index=True)
#volcano(da_df, group1='Low', group2='High', title='Low vs. High')

###da_df = my_net.Pathway_differential_activity(X=X, fdr_target=0.05, group1='Immune(-)', group2='Immune(+)', mode='vanilla')
###print(da_df.head(20))
###pd.DataFrame(da_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/DEPathways/immune(-)_immune(+).csv",index=True)
#volcano(da_df, group1='Low', group2='High', title='Low vs. High')


da_df = my_net.Pathway_differential_activity(X=X, fdr_target=0.05, group1='Graphist (-)', group2='Graphist (+)', mode='vanilla')
print(da_df.head(20))
pd.DataFrame(da_df).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/DEPathways/graphist(-)_graphist(+).csv",index=True)
#volcano(da_df, group1='Low', group2='High', title='Low vs. High')
