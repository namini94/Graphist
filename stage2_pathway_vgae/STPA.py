...
class SVEGA(nn.Module):
    def __init__(self, input_dim, dropout, n_gmvs, z_dropout, gmt_paths: Union[list,str] =None, add_nodes: int = 1,min_genes: int = 0,
                max_genes: int =5000,
                positive_decoder: bool = True, exp_paths: Union[list,str] =None, regularizer: str = 'mask', head_num=1, scale=1 ):
        ...
        self.mu = nn.Parameter(torch.zeros(n_gmvs), requires_grad=False)
        self.sigma = nn.Parameter(torch.ones(n_gmvs), requires_grad=False)
        ...
        self.encoder_st = GraphEncoder(input_dim, n_gmvs, head=head_num)
        #=========== decoder ================
...
# Create a DataLoader
#loader = DataLoader(data, batch_size=128, shuffle=True)  # Example with batch_size=2
...
def train(loader,model):
    ...
    #            max_genes,
    #            positive_decoder, exp_paths, regularizer, head_num=1, scale=1)
    #DNN.cuda()
    ...
    for data in loader:
        ...
        #if adj_matrix.dim() == 3:
        #    print('Nami')
        #    adj_matrix = adj_matrix.squeeze(0)  # Remove extra dimension
        #    adj_matrix=adj_matrix.view(-1, adj_matrix.size(2))
        #print(adj_matrix.dim())
        #data = Variable(data).cuda()
        #label = Variable(label).cuda()
        optimizer.zero_grad()
        p_z_given_x_a, z, X_rec = model(data.x,data.edge_index)
        #p_z_given_x_a, z, X_rec  = DNN(data,adj_matrix[0,0:])
        #p_z_given_x_a = DNN.encode(data, adj_matrix,adj_values)
        # Sample from the posteriors with reparametrization
        #z = p_z_given_x_a.rsample()
        # print(z)
        #X_rec = DNN.decode(z)
        ...
        reconstructionLoss = mse_loss(data.x,X_rec)
        #dose_hat, x_hat, mu, logvar, cutpoints = DNN(data)
        # KL Divergence
        kl_loss_ = p_z_given_x_a.log_prob(z) - model.z_prior.log_prob(z)
        kl_loss = kl_loss_.mean()
        loss = torch.mean(reconstructionLoss + kl_loss)
        ...
        #self.decoder.sparse_layer[0].apply(clipper)
            model.decoder._positive_weights()
...
def test(loader,model):           
    ...
    for data in loader:
        ...
        #label = Variable(label).cuda()
        p_z_given_x_a, z, X_rec  = model(data.x,data.edge_index)
        pred = np.array(X_rec.cpu().detach())
...
train_loader = DataLoader(data_list, batch_size=1, shuffle=True)  # Example with batch_size=2
...
#train(train_loader)
#pd.DataFrame(res).to_csv("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/res.csv",index=True)

#plt.figure(figsize=[4,4])
#plt.scatter(x=GroundTruth,y=np.array(pd.DataFrame(res).iloc[-1,2]),c='black')
...
#plt.savefig('/Users/naminiyakan/Documents/VEGA_Regress/plots_mycode/GT_PRED.pdf', format='pdf', dpi=300, bbox_inches='tight')


#print("Mean squared error: %.3f" % mean_squared_error(GroundTruth,np.array(pd.DataFrame(res).iloc[-1,2])))
#print('R2 Variance score: %.3f' % r2_score(GroundTruth,np.array(pd.DataFrame(res).iloc[-1,2])))
...
#plt.scatter(pd.DataFrame(res).iloc[:,0], pd.DataFrame(res).iloc[:,1])
#plt.plot(pd.DataFrame(res).iloc[:,0], pd.DataFrame(res).iloc[:,1])
#plt.savefig('/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/Model_loss_we0.pdf', format='pdf', dpi=300, bbox_inches='tight')
...
# plotting scatter and pyplot
#plt.scatter(pd.DataFrame(res).iloc[:,0], pd.DataFrame(res).iloc[:,1])
#plt.plot(pd.DataFrame(res).iloc[:,0], pd.DataFrame(res).iloc[:,2], label='Training')
#plt.plot(pd.DataFrame(res).iloc[:,0], pd.DataFrame(res).iloc[:,3], label='Test')
#plt.legend(loc='best')
#plt.savefig('/Users/naminiyakan/Documents/VEGA/No_Node/plots_mycode/Acc_we7.pdf', format='pdf', dpi=300, bbox_inches='tight')



model.save('/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/trained_models/', save_adata=False, save_history=False)

model = SVEGA.load('/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Graphencode/trained_models/')

#dataset = Radbio_Dataset(dataset_size=2518,train=True, ratio=1)
#x,y,d = dataset.data
#print(x,y)
#x = torch.from_numpy(x)
latent = model.encode(data.x)
