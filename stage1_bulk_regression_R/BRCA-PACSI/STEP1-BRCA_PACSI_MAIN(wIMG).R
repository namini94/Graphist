###############################################BRCA-PACSI#####################################
tissue_positions<-read.csv(file = "/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/outs/spatial/tissue_positions_list.csv",row.names = 1,header = F)
...
set.seed(2022)
matrix_dir="~/Documents/BulkToST/Dataset/BRCA-PACSI/outs"
...
cd<-Matrix::Matrix(cd, sparse = TRUE,nrow = 17943, ncol = 2518, dimnames = names_dims)
...
write.csv(cd,file="/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/processed/STcount.csv",quote = F,row.names = T)
write.csv(pos_cor,file = "/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/processed/Pos.csv",quote = F,row.names = T)
...
write.csv(tissue_mask,file="/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/processed/tissue_mask.csv",quote = F,row.names = F)
write.csv(colnames(cd),file="/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/processed/Gene_names.csv",quote = F,row.names = F)
write.csv(rownames(cd),file="/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/processed/Barcodes.csv",quote = F,row.names = F)
...
#######Network_Construction (Morphological Features added)
neighbors<-read.csv(file = "/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/processed/neighbors.csv",header = T,row.names = 1)
...
Morph_features<-read.csv(file = "/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Morph-ResNet50.csv",row.names = 1)
library(lsa)
Morph_features<-t(Morph_features)
sim_morph<-(abs(cosine(Morph_features))+cosine(Morph_features))/2
diag(sim_morph)<-0
colnames(sim_morph)<-rownames(cd)
rownames(sim_morph)<-rownames(cd)

library(Matrix)  # For sparse matrices and graph Laplacian

# Function to compute the graph Laplacian
compute_laplacian <- function(W) {
...
lapla_sim_graph<-compute_laplacian(sim_graph)
lapla_sim_morph<-compute_laplacian(sim_morph)

lapla_tot<-(lapla_sim_graph+lapla_sim_morph)/2

sim_tot<-diag(diag(lapla_tot))-lapla_tot

sim_hada<-sim_graph + sim_morph - (sim_graph * sim_morph)

library(dplyr)
# Remove Duplicate columns
cd <- cd[, !colnames(cd) %in% c("HSPA14","TBCE")]
...
Seurat_tmp@graphs$RNA_snn<-as.Graph(sim_hada)
...
Save_file = c("/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Scissor_BRCA-PACSI_DEATH_SURVDEATHPheno.RData")
family = c("cox")
...
scanpy_norm<-read.csv(file='/Users/naminiyakan/Documents/BulkToST/Dataset/BRCA-PACSI/processed/Normalized_BRCA_PACSI.csv',header = T,row.names = 1)
...
plt <- ggplot2::ggplot(data = dat) +
  ggplot2::geom_point(alpha=1,ggplot2::aes(x = emb1, y = emb2,
...
Scissor_select<-as.matrix(Scissor_select)
Scissor_select<-as.numeric(Scissor_select)
names(Scissor_select)<- colnames(sc_exprs)
write.csv(Scissor_select,file = "/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/selected_al3e-2.csv",quote = F,row.names = TRUE)


selection<-matrix(0,length(Scissor_select),1)
colnames(selection)<-c("selection")
selection[,1]<-Scissor_select
rownames(selection)<-colnames(Seurat_tmp)
Seurat_tmp <- AddMetaData(Seurat_tmp, selection)

Idents(Seurat_tmp) <- Seurat_tmp$selection

de_results <- FindMarkers(Seurat_tmp, 
                          ident.1 = "1",
                          ident.2 = "2",
                          min.pct = 0.1
                          #logfc.threshold = 1.25
                          )
write.csv(de_results,file = "/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/BRCA_PACSI_DE_Res.csv",quote = F,row.names = TRUE)

#VlnPlot(Seurat_tmp, features = c("S100A6", "LAMC2", "KRT17"))

##############Comparison Between immune(-) and immune(+)
compare<-read.csv(file="/Users/naminiyakan/Documents/BulkToST/Res-BRCA-PACSI/Comparison-immune(-)-immune(+).csv",row.names = 1,header = T)
rownames(compare)<-colnames(Seurat_tmp)

Seurat_tmp <- AddMetaData(Seurat_tmp, compare)

Idents(Seurat_tmp) <- Seurat_tmp$immune_groups

compare_de_results <- FindMarkers(Seurat_tmp, 
                                  ident.1 = "immune(+)",
                                  ident.2 = "immune(-)"
                                  #min.pct = 0.1
                                  #logfc.threshold = 1.25
